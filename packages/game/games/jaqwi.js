...
module.exports = class {
   ...
   * @private
   * @param theme {string}
   * @returns {Promise<null | {_id: string, mean: string}>}
   */
  getAnswer(theme) {
    const queryArgs = [
      ['_id', { $nin: this.ROOM.game.done }],
      ['theme', new RegExp('(,|^)(' + theme + ')(,|$)')],
      ['type', Const.KOR_GROUP],
      ['flag', { $lte: 7 }]
    ];

    return new Promise((resolve) => {
      this.DB.kkutu['ko'].find(...queryArgs).on(($res) => {
        if (!$res || !$res.length) return resolve(null);

        let len = $res.length;
        while (len > 0) {
          const randomIndex = Math.floor(Math.random() * len);
          const pickedWord = $res[randomIndex];
          if (pickedWord._id.length >= 2 && (pickedWord.type === 'INJEONG' || pickedWord.mean.length >= 0)) {
            return resolve(pickedWord);
          }
  ...
  async roundReady() {
    ...
    // 테마 (어인정 주제)
    const pickedThemeCount = this.ROOM.opts.injpick.length;
    this.ROOM.game.themeBonus = 0.3 * Math.log(0.6 * pickedThemeCount + 1);
    ...
    this.ROOM.game.theme = this.ROOM.opts.injpick[Math.floor(Math.random() * pickedThemeCount)];

    const answer = await this.getAnswer(this.ROOM.game.theme);

    if (!this.ROOM.game.done) return;

    // $ans가 null이면 골치아프다...
    this.ROOM.game.late = false;
    this.ROOM.game.answer = answer || {};
    this.ROOM.game.done.push(answer._id);

    if (answer.mean.length <= 20) {
      answer.mean = getConsonants(answer._id, Math.round(answer._id.length / 2));
    }
    this.ROOM.game.hint = getHint(answer);
  ...
  turnStart() {
    ...
    for (const robot of this.ROOM.game.robots) {
      this.ROOM.readyRobot(robot);
    }
  }
  ...
  submit(client, text) {
    ...
    if (play) {
      if (this.ROOM.game.primary + this.ROOM.game.giveup.length >= this.ROOM.game.seq.length) {
        clearTimeout(this.ROOM.game.hintTimer);
        clearTimeout(this.ROOM.game.hintTimer2);
        clearTimeout(this.ROOM.game.qTimer);
        this.ROOM.turnEnd();
