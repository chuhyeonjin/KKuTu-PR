...
const {Agent} = require("./collection");
...
describe('RedisTable 테스트', () => {
    ...
    test('putGlobal 테스트', done => {
        ...
        const agent = new Agent('Redis', {'zadd': zaddMock});
        const table = new agent.Table(redisKey);
    ...
    test('getGlobal 테스트', done => {
        ...
        const agent = new Agent('Redis', {'zrevrank': zrevrankMock});
        const table = new agent.Table(redisKey);
    ...
    test('getPage 테스트', done => {
        ...
        const agent = new Agent('Redis', {'zrevrange': zrevrangeMock});
        const table = new agent.Table(redisKey);
    ...
    test('getSurround 테스트', done => {
        ...
        const zrevrankMock = jest.fn((_, callback) => { callback(undefined, data.rank) });
        const zrevrangeMock = jest.fn((_, callback) => { callback(undefined, data.zrevrangeResult) });
        const agent = new Agent('Redis', {'zrevrank': zrevrankMock, 'zrevrange': zrevrangeMock});
        const table = new agent.Table(redisKey);
